...
AzureMediaService.prototype.getChannel = function ( channelId, cb ) {

  var params = { id : channelId }

  return this.request.get('Channels', params, cb)

}

AzureMediaService.prototype.resetChannel = function ( channelId, cb ) {
  ...
  return this.request.post('Channels', params, (err, res) => {
    if (err) cb(err)
    if (res.statusCode === 202) {
      cb(err, {
        operationId: res.headers['operation-id']
      })
    } else {
      cb(new Error('Unexpected statusCode (expected 202, actual: ' + res.statusCode.toString() + ')'))
    }
  })

}

AzureMediaService.prototype.getOperationState = function ( operationId, cb ) {

  var params = { id : operationId }

  return this.request.get('Operations', params, cb)
